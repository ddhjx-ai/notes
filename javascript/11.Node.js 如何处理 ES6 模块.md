JavaScript 语言，有两种格式的模块。

- ES6 模块，简称 ESM；
  - ES6 模块使用`import`和`export`。
- Node.js 专用的 CommonJS 模块，简称 CJS。这两种模块不兼容。
...
​	Node.js 要求 ES6 模块采用 `.mjs`后缀文件名。也就是说，脚本文件里面使用 import 或者 export 命令，那么就必须采用 .mjs 后缀名。Node.js 遇到 .mjs 文件就人文它是 ES6 模块，默认启用严格模式，不必在每个模块文件顶部指定 use strict。
